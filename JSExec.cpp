...
ASTNode resolveFunc(ScopeContainer &scopeBox, bool newvalExists, ASTNode newval)
{
	...
	do {
		if (ptrScope->ScopeArray.find(op1Str) != ptrScope->ScopeArray.end()) {
			...
			ASTNode* curValue = (*resolveRuntimeObject(ptrScope->ScopeArray[op1Str]).ASTArray)[op2Str];
			if (curValue != nullptr) {
				delete curValue;
			}
	...
	ASTNode* curValue = (*resolveRuntimeObject(op1).ASTArray)[op2Str];
	if (curValue != nullptr) {
		delete curValue;
	}
...
ASTNode resolve(ASTNode& op, Scope& scope) {
	if (op.scopeBox.scope__ != nullptr) {
...
ASTNode Let(ASTNode op1, ASTNode op2, Scope& scope, AbstractSyntaxTree& ast) {
	...
	if (op1.scopeBox.scope__ != nullptr) {
...
ASTNode Dot(ASTNode op1, ASTNode op2, Scope& scope, AbstractSyntaxTree& ast) {
	...
	retval.scopeBox = ScopeContainer();
	retval.scopeBox.scope__ = &scope;
	retval.scopeBox.op1__ = new ASTNode(op1);
	retval.scopeBox.op2__ = new ASTNode(op2);
	/*}
	else {
...
ASTNode PredefinedClearCanvas(vector<ASTNode> args, Scope& scope) {
	...
	if (canvasEl->bitmap && canvasEl->bitmap != nullptr) {
		for (int i = 0, len = 800 * 400 * 4; i < len; i++) {
			canvasEl->bitmap[i] = 0;
		}
	}
	return ASTNode();
...
void drawLineOnCanvas(DOMNode* canvasEl, vector<ASTNode> knots, ASTNode matrix) {
	...
	HRESULT hr = pRenderTarget2->CreateBitmap(D2D1::SizeU(800, 400), canvasEl->bitmap, 800 * 4, D2D1::BitmapProperties(D2D1::PixelFormat(DXGI_FORMAT_R8G8B8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED)), &pBitmap);
	...
	if (SUCCEEDED(hr))
	{
		...
		canvasEl->bitmap = new BYTE[800 * 400 * 4];
		pWICBitmap->CopyPixels(NULL, 800 * 4, 800 * 400 * 4, canvasEl->bitmap);
	}
...
ASTNode PredefinedDrawText(vector<ASTNode> args, Scope& scope) {
	...
	string text = resolveString(resolveRuntimeObject(args[1]).getString());

	string strToLog = text;
	string replacedString = "";
	for (int j = 0, len = strToLog.size(); j < len; j++) {
		if (j < len - 1 && strToLog[j] == '\\' && strToLog[j + 1] == 'n') {
			replacedString += "\n";
...
ASTNode PredefinedSqrt(vector<ASTNode> args, Scope& scope) {
	ASTNode num = resolve(args[0], scope);
	return ASTNode((long double)sqrt(num.getNumber()));
}
...
ASTNode evaluate(AbstractSyntaxTree ast, Scope& args) {
	...
	if (retval.scopeBox.scope__ != nullptr) {
	...
	while (i < len) {
		...
		if (expr[i] == '(') {
			...
			if (value && value != nullptr && key != "") {
				...
				else if (value->ASTNodeString != "" && !isNaN(value->ASTNodeString)) {
			...
			value = new ASTNode;
			key_or_value = "key";
			if (curBraceNode->parent && curBraceNode->parent != nullptr) {
				curBraceNode = curBraceNode->parent;
			}
			else {
				break;
...
ASTNode* parseArrayLiteral(string expr, int& i, Scope& args) {
	...
	while (i < len) {
		...
		if ((expr[i] == '\'' || expr[i] == '\"')) {
			...
			if (curElement && curElement != nullptr) {
...
ASTNode* parseNode(ASTNode* node, Scope& args) {
	...
	if (retval->scopeBox.scope__ != nullptr) {
		while (retval->scopeBox.scope__ != nullptr) {
			retval = &resolveFunc(retval->scopeBox, false, ASTNode());
		}
	}
	retval = parseASTNode(retval);
			...
			if (curParent->parent->funcName != "" || (prevAST.getString() != "" && prevAST.getType() != ASTBoolNode) || prevAST.getType() == ASTInstanceNode) {
				...
				if (curParent->childNodes.size() == 1) {
					...
					for (int i = 0, len = curParent->childNodes.size(); i < len; i++) {
						...
						else if (node->scopeBox.scope__ != nullptr) {
							while (node->scopeBox.scope__ != nullptr) {
